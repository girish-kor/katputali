...
- **Mitigation:** ward items (neem leaf bundle, *kalava* red thread) are consumable pickups placed near the tainted rooms; using one (interact from inventory) reduces the meter by a fixed amount and is consumed. **M4 grey-box simplification:** the real flow presumes the Tab inventory bar + slot-selection UI, which doesn't exist until M6 (see [[UI_UX]] §2) — until then, pressing `V` consumes the first ward item carried (if any) and mitigates, calling the same logic the real selection flow will call later.
